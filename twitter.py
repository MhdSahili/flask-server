...
class Tweet(base.Item):
    ...
    outlinksss = base._DeprecatedProperty('outlinksss', lambda self: ' '.join(
        self.outlinks) if self.outlinks else '', 'outlinks')
...
class _CLIGuestTokenManager(GuestTokenManager):
    def __init__(self):
        ...
        dir = os.path.join(cacheHome, 'PremierLeague')
...
class _TwitterAPIScraper(base.Scraper):
    ...
    def _ensure_guest_token(self, url=None):
        if self._guestTokenManager.token is None:
            ...
            r = self._get(self._baseUrl if url is None else url, headers={
                          'User-Agent': self._userAgent}, responseOkCallback=self._check_guest_token_response)
            ...
            if not self._guestTokenManager.token:
                ...
                r = self._post('https://api.twitter.com/1.1/guest/activate.json', data=b'',
                               headers=self._apiHeaders, responseOkCallback=self._check_guest_token_response)
                o = r.json()
        ...
        self._session.cookies.set('gt', self._guestTokenManager.token, domain='.twitter.com',
                                  path='/', secure=True, expires=self._guestTokenManager.setTime + 10800)
                ...
                for entry in entries:
                    ...
                    # E.g. 'offensive' replies button
                        ...
                        newBottomCursorAndStop = (entry['content']['operation']['cursor']['value'],
                                                  entry['content']['operation']['cursor'].get('stopOnEmptyResponse', False))
    ...
    def _instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
        if 'tweet' in entry['item']['content']:
            ...
            if entry['item']['content']['tweet']['id'] not in obj['globalObjects']['tweets']:
                ...
                return
            tweet = obj['globalObjects']['tweets'][entry['item']
                                                   ['content']['tweet']['id']]
        elif 'tombstone' in entry['item']['content']:
            # E.g. deleted reply
            if 'tweet' not in entry['item']['content']['tombstone']:
                return
    ...
    def _tweet_to_tweet(self, tweet, obj):
        ...
        if tweet['entities'].get('urls'):
            kwargs['outlinks'] = [u['expanded_url']
                                  for u in tweet['entities']['urls']]
            kwargs['tcooutlinks'] = [u['url']
                                     for u in tweet['entities']['urls']]
                ...
                if medium['type'] == 'photo':
                    ...
                    for variant in medium['video_info']['variants']:
                        variants.append(VideoVariant(
                            contentType=variant['content_type'], url=variant['url'], bitrate=variant.get('bitrate')))
                    mKwargs = {
            ...
            if inReplyToUserId == kwargs['user'].id:
                ...
                for u in tweet['entities']['user_mentions']:
                    if u['id_str'] == tweet['in_reply_to_user_id_str']:
                        kwargs['inReplyToUser'] = User(username=u['screen_name'], id=u['id'] if 'id' in u else int(
        ...
        if tweet['entities'].get('user_mentions'):
            kwargs['mentionedUsers'] = [User(username=u['screen_name'], id=u['id'] if 'id' in u else int(
                u['id_str']), displayname=u['name']) for u in tweet['entities']['user_mentions']]
        ...
        if tweet.get('place'):
            kwargs['place'] = Place(tweet['place']['full_name'], tweet['place']['name'], tweet['place']
                                    ['place_type'], tweet['place']['country'], tweet['place']['country_code'])
        ...
        if tweet['entities'].get('hashtags'):
            kwargs['hashtags'] = [o['text']
                                  for o in tweet['entities']['hashtags']]
        if tweet['entities'].get('symbols'):
            kwargs['cashtags'] = [o['text']
                                  for o in tweet['entities']['symbols']]
        return Tweet(**kwargs)
    ...
    def _user_to_user(self, user):
        ...
        if user['entities']['description'].get('urls'):
            kwargs['descriptionUrls'] = [{'text': x.get('display_url'), 'url': x['expanded_url'], 'tcourl': x['url'], 'indices': tuple(
                x['indices'])} for x in user['entities']['description']['urls']]
        ...
        if 'url' in user['entities']:
            kwargs['linkUrl'] = (user['entities']['url']['urls'][0].get(
                'expanded_url') or user.get('url'))
...
class TwitterSearchScraper(_TwitterAPIScraper):
    ...
    def cli_setup_parser(cls, subparser):
        ...
        subparser.add_argument('query', type=base.nonempty_string(
            'query'), help='A Twitter search string')
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def _get_entity(self):
        ...
        params = {'variables': json.dumps(
            {fieldName: self._username, 'withHighlightedLabel': True}, separators=(',', ':'))}
        obj = self._get_api_data(endpoint, params=urllib.parse.urlencode(
        ...
        return User(
            ...
            descriptionUrls=[{'text': x.get('display_url'), 'url': x['expanded_url'], 'tcourl': x['url'], 'indices': tuple(
                x['indices'])} for x in user['legacy']['entities']['description']['urls']],
    ...
    def cli_setup_parser(cls, subparser):
        ...
        subparser.add_argument('--user-id', dest='isUserId', action='store_true',
                               default=False, help='Use user ID instead of username')
        subparser.add_argument('username', type=username,
...
class TwitterHashtagScraper(TwitterSearchScraper):
    ...
    @classmethod
    def cli_setup_parser(cls, subparser):
        subparser.add_argument('hashtag', type=base.nonempty_string(
            'hashtag'), help='A Twitter hashtag (without #)')
...
class TwitterTweetScraper(_TwitterAPIScraper):
    ...
    def __init__(self, tweetId, mode=TwitterTweetScraperMode.SINGLE, **kwargs):
        ...
        self._mode = mode
        super().__init__(
            f'https://twitter.com/i/web/status/{self._tweetId}', **kwargs)
    ...
    def get_items(self):
        ...
        if self._mode is TwitterTweetScraperMode.SINGLE:
            obj = self._get_api_data(
                f'https://twitter.com/i/api/2/timeline/conversation/{self._tweetId}.json', params)
    ...
    def cli_setup_parser(cls, subparser):
        ...
        group.add_argument('--scroll', action='store_true',
                           default=False, help='Enable scrolling in both directions')
...
class TwitterListPostsScraper(TwitterSearchScraper):
    ...
    @classmethod
    def cli_setup_parser(cls, subparser):
        subparser.add_argument('list', type=base.nonempty_string(
            'list'), help='A Twitter list ID or a string of the form "username/listname" (replace spaces with dashes)')
...
class TwitterTrendsScraper(_TwitterAPIScraper):
    ...
    def get_items(self):
        ...
        obj = self._get_api_data(
            f'https://twitter.com/i/api/2/guide.json', params)
        for instruction in obj['timeline']['instructions']:
